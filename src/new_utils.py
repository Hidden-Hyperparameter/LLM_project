...
def query_llm(texts:str,quiet=True):
    print('[INFO] query LLM')
    # ZHH add code: if the texts is too long, then truncate
...
def WriteTag(file_name, tags):
    print('[INFO] Write Tag')
    dict_tags = {}
    if os.path.exists(TAG_DIR):
        with open(TAG_DIR, 'r',encoding='utf-8') as f:
            dict_tags = json.load(f)
    dict_tags[file_name] = tags
    with open(TAG_DIR, 'w',encoding='utf-8') as f:
        json.dump(dict_tags, f,indent=4,ensure_ascii=False)
